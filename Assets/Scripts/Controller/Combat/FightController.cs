...
public class FightController : MonoBehaviour
{
    ...
    public IEnumerator ShowAttackWithAbilityLogs()
    {
        ...
        dictionaryLog = CreateLogDictionary(attaquant.characterName, DropdownUpdater.instance.abilityStat1Dropdown.GetComponentInChildren<Text>().text,
            attackValue1.ToString(), "/ " + (abilityStat1 + bonus).ToString());

        bool success = true;

        if (attackValue1 <= abilityStat1 + bonus && attackValue1 > 5)
        {
            LogController.instance.AddAttaquantWinLog(dictionaryLog);
        }
        else if (attackValue1 <= abilityStat1 + bonus && attackValue1 <= 5)
        {
            LogController.instance.AddAttaquantCriticalWinLog(dictionaryLog);
        }
        else if (attackValue1 > abilityStat1 + bonus && attackValue1 <= 95)
        {
            LogController.instance.AddAttaquantFailLog(dictionaryLog);
            success = false;
        }
    ...
    public void RollDiceForStat()
    {
        ...
        int statWithBonus = stat;
        if (DropdownUpdater.instance.bonusStatInput.text != "")
        {
            statWithBonus += int.Parse(DropdownUpdater.instance.bonusStatInput.text);
        }

        Dictionary<string, string> log = CreateLogDictionary(attaquant.characterName,
            DropdownUpdater.instance.statDropdown.GetComponentInChildren<Text>().text,
            roll.ToString(), "/ " + statWithBonus.ToString());

        if (roll <= statWithBonus && roll > 5)
        {
            LogController.instance.AddAttaquantWinLog(log);
        }
        else if (roll <= statWithBonus && roll <= 5)
        {
            LogController.instance.AddAttaquantCriticalWinLog(log);
        }
        else if (roll > statWithBonus && roll <= 95)
        {
    ...
    public void RollBasicDice()
    {
        int maxValue = 0;
        if (DropdownUpdater.instance.diceInput.text != "")
        {
            maxValue = int.Parse(DropdownUpdater.instance.diceInput.text);
        }
        ...
        {
            int roll = RollDice(maxValue);

            int bonus = 0;
            if (DropdownUpdater.instance.bonusDiceInput.text != "")
            {
                bonus = int.Parse(DropdownUpdater.instance.bonusDiceInput.text);
            }
            int statWithBonus = maxValue + bonus;

            Dictionary<string, string> log;
            if (bonus != 0)
            {
                log = CreateLogDictionary(attaquant.characterName,
                    DropdownUpdater.instance.statDropdown.GetComponentInChildren<Text>().text,
                    roll.ToString() + " + " + bonus.ToString(), "/ " + statWithBonus.ToString());
            }
            else
            {
                log = CreateLogDictionary(attaquant.characterName,
            ...
            LogController.instance.AddAttaquantClassicalLog(log);
